...
var i18n = {
  ...
  ar: {
    ...
    s1Title: "اشرح الحوسبة الكمية",
    s1Desc: "بعبارات بسيطة للمبتدئين",
    s2Title: "اكتب خطة عمل",
    s2Desc: "لشركة تقنية ناشئة",
    s3Title: "عزّز إنتاجيتي",
    s3Desc: "استراتيجيات وروتين يومي",
    s4Title: "صحّح الكود",
    s4Desc: "ابحث عن أخطاء جافاسكريبت",
    ...
    tempLabel: "مؤقت",
    greeting: "مرحبًا",
    deletedToast: "تم حذف المحادثة",
    allDeletedToast: "تم حذف جميع المحادثات",
    nameSaved: "تم حفظ الإعدادات",
    confirmTitle: "تأكيد",
    fileTooBig: "الملف كبير جدًا (الحد الأقصى 10 ميجا)",
    copied: "تم النسخ!",
    copy: "نسخ",
    ...
    or: "أو",
    googleSign: "المتابعة بحساب Google",
    forgotPassword: "نسيت كلمة المرور؟",
    resetPassword: "إعادة تعيين كلمة المرور",
    resetDesc: "أدخل إيميلك لإرسال رابط إعادة التعيين",
    sendReset: "إرسال رابط",
    resetSent: "تم الإرسال! تحقق من إيميلك.",
...
// ==========================================
//  SIDEBAR — FIX: فصل الإغلاق عن الفتح
// ==========================================
...
// ==========================================
//  APP UI — FIX: الإعدادات واللغة وتسجيل الخروج
// ==========================================
function initAppUI() {
  ...
  if (overlay) overlay.onclick = function(e) {
    // فقط أغلق إذا الضغط على الأوفرلاي نفسه وليس على السايدبار
    if (e.target === overlay) {
      closeSidebar();
    }
  };
  ...
  if (settingsBtn) {
    ...
    settingsBtn.onclick = function (e) {
      ...
      markSidebarButtonClick();
      closeSidebar();
      // تأخير بسيط لضمان إغلاق السايدبار أولاً
...
function startTempChat() {
  ...
  conversations.push({
    id: id,
    title: currentLang === "ar" ? "محادثة مؤقتة" : "Temporary Chat",
    messages: [],
    temp: true,
  });
  ...
  for (var i = 0; i < pendingFiles.length; i++) {
    (function (f, idx) {
      ...
      item.innerHTML =
        ...
        (f.isImage ? "" : "📄 ") +
        escapeHTML(f.name) +
        '</span><button type="button" class="file-preview-remove">✕</button>';
...
async function generateImage(prompt, conv) {
  ...
  try {
    var imagePrompt = prompt
      .replace(/^(ارسم|صمم|اصنع|ولّد|أنشئ|انشئ|draw|paint|generate|create|make|design|sketch)\s*/i, "")
      .replace(/^(me\s+|لي\s+)?/i, "")
      .replace(/^(a |an |the )?(image|picture|photo|illustration|صورة|رسمة|لوحة)\s*(of |for |about |عن |ل)?/i, "")
      .trim() || prompt;
    var imageUrl =
...
// قائمة النماذج البديلة في حالة فشل النموذج الأساسي
var FALLBACK_MODELS = [
...
// نموذج بديل في حالة فشل الأساسي
